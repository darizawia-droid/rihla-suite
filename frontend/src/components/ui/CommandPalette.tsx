...
const NAV_ITEMS: NavItem[] = [
  ...
  { label: 'Budget Tracker',   path: '/budget-tracker',     icon: Calculator,  group: 'Performance' },
  { label: 'Scoring Fournisseurs', path: '/supplier-scoring', icon: Trophy,     group: 'Performance' },
  { label: 'Allotments',       path: '/allotments',         icon: Hotel,       group: 'Logistique' },
  { label: 'Export Excel',     path: '/excel-export',       icon: Receipt,     group: 'Logistique' },
  { label: 'Passagers',        path: '/passengers',         icon: Users,       group: 'Logistique' },
  { label: 'WhatsApp Chat',    path: '/whatsapp',           icon: Zap,         group: 'Communication' },
  { label: 'Vols',             path: '/flights',            icon: Bus,         group: 'Logistique' },
...
export function CommandPalette({ open, onClose }: Props) {
  ...
  const matchedActions = useMemo(() => {
    if (!query || query.startsWith('ai:')) return QUICK_ACTIONS
  ...
  const matchedNav = useMemo(() => {
    if (!query || query.startsWith('ai:')) return NAV_ITEMS
    const q = query.toLowerCase()
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    if (e.key === 'Enter') {
      if (query.startsWith('ai:')) {
        window.dispatchEvent(new CustomEvent('genius-ai-query', { detail: query.slice(3).trim() }))
        onClose()
        return
      }
  ...
  const renderAI = () => {
    if (!query.startsWith('ai:')) return null
    return (
      <Section label="Genius AI" icon={<Sparkles size={9} className="text-amber-400" />}>
        <Row
          active={true}
          onClick={() => {
            window.dispatchEvent(new CustomEvent('genius-ai-query', { detail: query.slice(3).trim() }))
          ...
          icon={<Brain size={14} className="text-amber-600 animate-pulse" />}
          title={`Demander à l'IA : ${query.slice(3)}`}
          subtitle="Générez des itinéraires, analysez des budgets ou cherchez des données"
          tail={<Kbd>↵</Kbd>}
        />
      </Section>
    )
      ...
      <div className="relative w-full max-w-2xl bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 overflow-hidden animate-in fade-in slide-in-from-top-4 duration-200">
        ...
        <div className="flex items-center gap-3 px-5 py-4 border-b border-slate-100 dark:border-slate-800">
          ...
          <input
            ...
            placeholder='Tapez "ai:" pour demander à Genius, ou recherchez un projet…'
        ...
        {/* Results */}
        <div className="max-h-[440px] overflow-y-auto py-2 custom-scrollbar">
          {renderAI()}
          {!query.startsWith('ai:') && allResults.length === 0 && (
        ...
        <div className="px-5 py-2.5 border-t border-slate-100 dark:border-slate-800 flex items-center gap-4 text-[11px] text-slate-400">
          ...
          <span className="flex items-center gap-1.5 font-bold text-rihla/60">
            Astuce: Tapez "ai:" pour Genius
          </span>
        </div>
      </div>
    </div>
...
function Row({
  ...
  return (
    ...
    >
      <div className={clsx('w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 shadow-sm', iconBg)}>
        {icon}
      </div>
