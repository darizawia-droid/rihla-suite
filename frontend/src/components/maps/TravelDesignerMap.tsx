...
function GlobalView({
  ...
  useEffect(() => {
    ...
    const tick = (ts: number) => {
      ...
      setProgress(prev => {
        const next = prev + dt / 10 // 10s loop
        if (next >= 1) return 0
        return next
      })
  ...
  const travelerPos: [number, number] = useMemo(() => {
    ...
    const target = Math.max(0, Math.min(progress, 1)) * totalSteps
    let acc = 0
      ...
      <div className="col-span-12 lg:col-span-9 relative h-[520px]">
        ...
        <div className="absolute top-4 right-4 z-[1000]">
          <button
            ...
            className="flex items-center gap-2 px-3 py-1.5 bg-white/95 backdrop-blur-md border border-slate-200/70 rounded-xl shadow-md text-[11px] font-bold uppercase text-slate-700 hover:text-rose-700 transition"
          >
            {playing ? '⏸ Pause anim' : '▶️ Play anim'}
          </button>
        </div>
      </div>
