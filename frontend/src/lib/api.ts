...
export const itinerariesApi = {
  ...
  generateFull: (data: { project_id: string; prompt: string; duration_days: number; tone?: string }) =>
    api.post('/itineraries/generate-full', data),
...
export const roomingApi = {
  exportExcel: (projectId: string) => 
    api.get(`/rooming/export-excel/${projectId}`, { responseType: 'blob' }),
  exportPdf: (projectId: string) => 
    api.get(`/rooming/export-pdf/${projectId}`, { responseType: 'blob' }),
}
...
export interface RecomputePayload {
  ...
  foc_count?: number
  markup_pct?: number
  bus_total_cost?: number
  tour_leader_cost?: number
  guide_cost?: number
  guide_local_cost?: number
