...
export function ProposalViewPage() {
  ...
  const isLive = isWon // Future: also check if current date is within travel dates
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white font-sans selection:bg-rihla/30">
            ...
            <div className="animate-in fade-in slide-in-from-bottom-8 duration-1000">
               <div className="flex items-center gap-3 mb-6">
                 <div className="px-4 py-1.5 bg-rihla/20 backdrop-blur-xl border border-rihla/30 rounded-full text-[10px] font-black uppercase tracking-[0.3em] text-rihla">
                    {isLive ? '🔴 COMPAGNON DE VOYAGE LIVE' : 'Expérience Exclusive S\'TOURS'}
                 </div>
               ...
               <div className="flex gap-12 items-center">
                  <div className="flex gap-10">
                     ...
                     {isLive && project.arrival_flight && (
                       <Stat icon={Plane} label="Vol Arrivée" value={project.arrival_flight} />
                     )}
                  </div>
                  <button 
                    onClick={() => {
      ...
      {/* ── FIELD STAFF SECTION ─────────────────────────────── */}
      {(project.assigned_guide || project.assigned_driver) && (
        <section className="max-w-7xl mx-auto py-32 px-10 border-t border-white/5">
           <div className="text-center mb-20">
              <h2 className="text-5xl font-black tracking-tighter mb-6">Vos Experts Dédiés</h2>
              <p className="text-white/40 max-w-xl mx-auto font-medium">
                Pour vous garantir une immersion totale et une logistique parfaite, notre équipe d'experts vous accompagne à chaque étape.
              </p>
           </div>
           
           <div className="flex justify-center gap-12">
              {project.assigned_guide && (
                <div className="w-80 group">
                   <div className="relative aspect-[3/4] rounded-[40px] overflow-hidden mb-8 border border-white/10 group-hover:border-rihla/50 transition-colors">
                      <img src={project.assigned_guide.image_url} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" alt="Guide" />
                      <div className="absolute inset-0 bg-gradient-to-t from-slate-950 via-transparent to-transparent" />
                      <div className="absolute bottom-8 left-8">
                         <p className="text-[10px] font-black text-rihla uppercase tracking-[0.2em] mb-2">Votre Guide Expert</p>
                         <h4 className="text-2xl font-black">{project.assigned_guide.name}</h4>
                      </div>
                   </div>
                   <div className="px-4 space-y-2 opacity-60 group-hover:opacity-100 transition-opacity">
                      <p className="text-xs font-medium flex items-center gap-2"><Mail size={14} className="text-rihla" /> {project.assigned_guide.email}</p>
                   </div>
                </div>
              )}

              {project.assigned_driver && (
                <div className="w-80 group">
                   <div className="relative aspect-[3/4] rounded-[40px] overflow-hidden mb-8 border border-white/10 group-hover:border-emerald-500/50 transition-colors">
                      <img src={project.assigned_driver.image_url} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" alt="Chauffeur" />
                      <div className="absolute inset-0 bg-gradient-to-t from-slate-950 via-transparent to-transparent" />
                      <div className="absolute bottom-8 left-8">
                         <p className="text-[10px] font-black text-emerald-500 uppercase tracking-[0.2em] mb-2">Votre Chauffeur Pro</p>
                         <h4 className="text-2xl font-black">{project.assigned_driver.name}</h4>
                      </div>
                   </div>
                   <div className="px-4 space-y-2 opacity-60 group-hover:opacity-100 transition-opacity">
                      <p className="text-xs font-medium flex items-center gap-2"><Mail size={14} className="text-emerald-500" /> {project.assigned_driver.email}</p>
                   </div>
                </div>
              )}
           </div>
      ...
      {/* ── FEEDBACK SECTION (POST-STAY) ───────────────────── */}
      {isWon && (
        <section className="max-w-3xl mx-auto py-32 px-10 text-center">
           <div className="bg-white/5 border border-white/10 rounded-[60px] p-16 space-y-8">
              <div className="w-20 h-20 bg-rihla/20 rounded-full flex items-center justify-center text-rihla mx-auto">
                 <Star size={40} fill="currentColor" />
              </div>
              <h2 className="text-4xl font-black tracking-tighter">Votre avis nous est précieux</h2>
              <p className="text-white/40 font-medium">
                Comment s'est déroulée votre expérience avec S'TOURS ? <br/> 
                Votre retour nous aide à viser l'excellence.
              </p>
              <div className="flex justify-center gap-4">
                 {[1,2,3,4,5].map(s => (
                   <button key={s} className="w-16 h-16 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center text-2xl hover:bg-rihla hover:text-white transition-all">
                     {s}
                   </button>
                 ))}
              </div>
              <textarea 
                placeholder="Un commentaire particulier sur votre guide ou vos hébergements ?"
                className="w-full bg-black/40 border border-white/10 rounded-3xl p-6 text-sm focus:outline-none focus:border-rihla"
                rows={4}
              />
              <button className="px-12 py-5 bg-rihla text-white text-[11px] font-black uppercase rounded-full shadow-2xl">
      ...
      {isLive && <SOSWidget projectId={project.id} />}
      {isLive && <LivePulseWidget pulse={project.live_pulse || []} />}
    </div>
  )
}

function LivePulseWidget({ pulse }: { pulse: any[] }) {
  const [isOpen, setIsOpen] = useState(false)

  // Mocking some pulse data if empty for demo
  const displayPulse = pulse.length > 0 ? pulse : [
    { id: '1', time: '09:00', author: 'Designer', msg: 'Bienvenue au Maroc ! Votre chauffeur vous attend à la porte 2.', type: 'welcome' },
    { id: '2', time: '11:30', author: 'Guide', msg: 'Check-in effectué à La Mamounia. Les bagages sont en chambre.', type: 'logistics' },
    { id: '3', time: '14:00', author: 'Designer', msg: 'Surprise : Une table VIP vous attend ce soir au Dar Yacout.', type: 'magic' },
  ]
  ...
  return (
    <>
      <button 
        ...
        className="fixed bottom-10 left-10 w-16 h-16 bg-rihla text-white rounded-full flex items-center justify-center shadow-2xl hover:scale-110 active:scale-95 transition-all z-[60] border-4 border-white/20"
      >
         <div className="relative">
            <Activity size={24} />
            <span className="absolute -top-2 -right-2 w-4 h-4 bg-emerald-500 rounded-full border-2 border-rihla animate-ping" />
         </div>
      </button>

      {isOpen && (
        <div className="fixed inset-0 z-[70] flex items-end sm:items-center justify-center p-0 sm:p-6 bg-slate-950/90 backdrop-blur-xl">
           <div className="w-full max-w-lg bg-slate-900 rounded-t-[40px] sm:rounded-[40px] border border-white/10 p-8 space-y-8 animate-in slide-in-from-bottom-10 duration-500">
              <div className="flex justify-between items-center">
                 <div className="flex items-center gap-3">
                    <div className="w-10 h-10 bg-rihla/20 rounded-xl flex items-center justify-center text-rihla">
                       <Zap size={20} />
                    </div>
                    <div>
                       <h3 className="text-xl font-black text-white">Live Trip Pulse</h3>
                       <p className="text-[10px] font-black text-white/40 uppercase tracking-widest">Flux de synchronisation terrain</p>
                    </div>
                 </div>
                 <button onClick={() => setIsOpen(false)}><X size={24} className="text-white/40" /></button>
              </div>

              <div className="space-y-6 max-h-[60vh] overflow-y-auto pr-2 custom-scrollbar">
                 {displayPulse.map((item, idx) => (
                   <div key={idx} className="relative pl-10 group">
                      {idx !== displayPulse.length - 1 && (
                        <div className="absolute left-4 top-8 bottom-0 w-0.5 bg-white/5 group-hover:bg-rihla/20 transition-colors" />
                      )}
                      <div className={clsx(
                        "absolute left-2 top-1 w-4 h-4 rounded-full border-2 border-slate-900 z-10",
                        item.type === 'magic' ? "bg-amber-500" : item.type === 'logistics' ? "bg-blue-500" : "bg-rihla"
                      )} />
                      
                      <div className="bg-white/5 border border-white/5 p-4 rounded-2xl group-hover:border-white/10 transition-all">
                         <div className="flex justify-between items-start mb-2">
                            <span className="text-[10px] font-black uppercase text-white/40 tracking-widest">{item.author}</span>
                            <span className="text-[10px] font-bold text-white/20 italic">{item.time}</span>
                         </div>
                         <p className="text-sm text-white/80 leading-relaxed font-medium">
                            {item.msg}
                         </p>
                         {item.type === 'magic' && (
                           <div className="mt-3 flex items-center gap-2 text-[10px] font-black text-amber-500 uppercase">
                              <Sparkles size={12} /> Moment d'exception
                           </div>
                         )}
                      </div>
                   </div>
              ...
              <div className="pt-4 border-t border-white/5">
                 <p className="text-[10px] text-center text-white/30 font-bold italic">
                   "Votre équipe est à votre écoute 24/7 pour un voyage sans couture."
                 </p>
              </div>
           </div>
        </div>
...
function SOSWidget({ projectId }: { projectId: string }) {
  const [isOpen, setIsOpen] = useState(false)
  ...
  return (
    <>
      <button 
        onClick={() => setIsOpen(true)}
        className="fixed bottom-10 right-10 w-20 h-20 bg-red-600 text-white rounded-full flex flex-col items-center justify-center shadow-2xl hover:scale-110 active:scale-95 transition-all z-[60] border-4 border-white/20 animate-pulse"
      >
         <AlertCircle size={24} />
         <span className="text-[8px] font-black uppercase tracking-tighter mt-1">SOS HELP</span>
      </button>

      {isOpen && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-6 bg-slate-950/90 backdrop-blur-xl">
           <div className="w-full max-w-md bg-slate-900 rounded-[40px] border border-white/10 p-8 space-y-6 animate-in zoom-in-95 duration-300">
              {!sent ? (
                <>
                  <div className="flex justify-between items-center">
                     <h3 className="text-xl font-black text-white">Assistance Immédiate</h3>
                     <button onClick={() => setIsOpen(false)}><X size={24} className="text-white/40" /></button>
                  </div>
                  <p className="text-xs text-white/60 font-medium leading-relaxed">Quel type de problème rencontrez-vous ? Votre équipe de terrain sera immédiatement alertée.</p>
                  
                  <div className="grid grid-cols-2 gap-3">
                     {[
                       { id: 'hotel', label: 'Hôtel', icon: Building2 },
                       { id: 'transport', label: 'Transport', icon: Plane },
                       { id: 'health', label: 'Santé', icon: ShieldCheck },
                       { id: 'quality', label: 'Service', icon: Star },
                     ].map(cat => (
                       <button 
                         key={cat.id}
                         ...
                         className={clsx(
                           ...
                           type === cat.id ? "bg-red-600 border-red-600 text-white" : "bg-white/5 border-white/10 text-white/60"
                         )}
                       >
                          <cat.icon size={20} />
                  ...
                  <textarea 
                    ...
                    className="w-full bg-black/40 border border-white/10 rounded-2xl p-4 text-sm text-white focus:outline-none h-24"
                  />

                  <button 
                    onClick={sendSos}
                    className="w-full py-4 bg-red-600 text-white font-black uppercase text-xs tracking-widest rounded-2xl shadow-xl hover:bg-red-700 transition-colors"
                  >
                    Lancer l'alerte SOS
                  </button>
                </>
                ...
                <div className="text-center py-10 space-y-6 animate-in zoom-in-95">
                   ...
                   <div>
                      <h4 className="text-2xl font-black text-white">Alerte Transmise</h4>
                      <p className="text-sm text-white/60 mt-2 leading-relaxed">
                        Votre guide et votre chauffeur ont reçu l'alerte.<br/>
