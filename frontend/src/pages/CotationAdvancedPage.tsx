...
const MOCK_PROJS = [
  { id: 'proj-demo1', name: 'Kingdom of Experiences (VIP Demo)', quotations: [{ id: 'quote-demo1', currency: 'USD', margin_pct: 12 }] },
  { id: 'proj-demo2', name: 'Imperial Cities B2B', quotations: [{ id: 'quote-demo2', currency: 'EUR', margin_pct: 10 }] }
]

const MOCK_FULL_VIEW: CotationFullView = {
  quotation: { margin_pct: 12, single_supplement: 350, currency: 'USD' },
  summary: { total_lines: 42, total_brackets: 6, total_terms: 13 },
  brackets: [
    { pax_min: 10, pax_max: 14, foc: 1, base_price: 1450, total_sales: 14500, margin_amount: 1740 },
    { pax_min: 15, pax_max: 19, foc: 1, base_price: 1320, total_sales: 19800, margin_amount: 2376 },
    { pax_min: 20, pax_max: 24, foc: 1, base_price: 1210, total_sales: 24200, margin_amount: 2904 },
    { pax_min: 25, pax_max: 29, foc: 1, base_price: 1140, total_sales: 28500, margin_amount: 3420 },
    { pax_min: 30, pax_max: 34, foc: 1, base_price: 1080, total_sales: 32400, margin_amount: 3888 },
    { pax_min: 35, pax_max: 40, foc: 2, base_price: 1050, total_sales: 36750, margin_amount: 4410 },
  ],
  terms: [
    { title: 'Inclusions', content: 'Hébergement 5 étoiles, transferts aéroport, bus privé, guides certifiés...' },
    { title: 'Exclusions', content: 'Vols internationaux, assurances, pourboires, dépenses personnelles...' },
    { title: 'Conditions d\'Annulation', content: 'Jusqu\'à 30 jours: sans frais. Entre 29 et 15 jours: 50%. Moins de 14 jours: 100%.' }
  ],
  lines: [
    { id: '1', day: 1, city: 'Marrakech', category: 'Hôtel', description: 'La Mamounia 5* - Chambre Deluxe', cost: 350 },
    { id: '2', day: 1, city: 'Marrakech', category: 'Transport', description: 'Transfert Aéroport', cost: 150 },
    { id: '3', day: 2, city: 'Marrakech', category: 'Activité', description: 'Visite guidée Médina + Palais Bahia', cost: 45 },
    { id: '4', day: 3, city: 'Agafay', category: 'Activité', description: 'Excursion 4x4 et dromadaire', cost: 120 },
  ]
}

const MOCK_VEHICLES: VehicleT[] = [
  { id: 'v1', label: 'Mercedes V-Class', type: 'Minivan VIP', capacity_min: 2, capacity_max: 6, rate_per_day: 150, currency: 'USD' },
  { id: 'v2', label: 'Mercedes Sprinter', type: 'Minibus', capacity_min: 8, capacity_max: 17, rate_per_day: 180, currency: 'USD' },
  { id: 'v3', label: 'Irizar i6 48 Pax', type: 'Autocar Touristique', capacity_min: 20, capacity_max: 48, rate_per_day: 350, currency: 'USD' },
]
...
export function CotationAdvancedPage() {
  ...
  async function refreshProjects() {
    try {
      ...
      let data = r.data
      if (!data || data.length === 0) data = MOCK_PROJS
      ...
      console.warn("API failed, using MOCK_PROJS")
      const projs: Project[] = []
      const allQuotes: Quote[] = []
      for (const p of MOCK_PROJS) {
        projs.push({ id: p.id, name: p.name })
        for (const q of p.quotations) {
          allQuotes.push({ id: q.id, project_id: p.id, currency: q.currency ?? 'EUR', margin_pct: Number(q.margin_pct ?? 0) })
        }
      }
      setProjects(projs)
      setQuotes(allQuotes)
  ...
  async function refreshFull(qid: string) {
    ...
    try {
      ...
      const data = r.data || MOCK_FULL_VIEW
      setView(data)
      const initialMarkup = Number(data.quotation?.margin_pct || 12)
      const initialSS = Number(data.quotation?.single_supplement || 350)
      const initialCur = data.quotation?.currency || 'USD'
      setRecomputeForm(prev => ({
        ...prev, markup_pct: initialMarkup,
        single_supplement: initialSS, currency: initialCur,
      }))
    } catch (e: any) {
      console.warn("API failed, using MOCK_FULL_VIEW")
      setView(MOCK_FULL_VIEW)
      setRecomputeForm(prev => ({ ...prev, markup_pct: 12, single_supplement: 350, currency: 'USD' }))
  ...
  async function refreshVehicles() {
    try {
      ...
      if (!r.data || r.data.length === 0) setVehicles(MOCK_VEHICLES)
      else setVehicles(r.data)
    } catch (e) {
      console.warn("API failed, using MOCK_VEHICLES")
  ...
  async function recompute() {
    ...
    try {
      ...
      console.warn("API failed, simulating recompute")
      setTimeout(() => {
        setMsg('grille recalculée (Mode Demo)')
        setBusy(false)
      }, 600)
    }
  }
