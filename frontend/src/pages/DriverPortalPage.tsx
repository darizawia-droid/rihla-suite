...
import { Car, Clock, MapPin, Phone, CheckCircle, AlertCircle, Navigation, Plane, ShieldAlert, Luggage } from 'lucide-react'
...
const MOCK_COURSES: Course[] = [
  ...
  },
  {
    ...
    flightNumber: 'AF1234', luggage: '4 valises larges, 2 cabines'
  },
  {
        ...
        {courses.map((course) => {
          ...
          return (
            <div key={course.id} style={{
              ...
              {/* Vol + Bagages */}
              {(course.flightNumber || course.luggage) && (
                <div style={{ margin: '10px 0', padding: '10px', background: 'rgba(22, 40, 169, 0.1)', borderRadius: 10, borderLeft: '3px solid #1628A9' }}>
                   {course.flightNumber && (
                     <div style={{ display: 'flex', items: 'center', gap: 6, fontSize: '0.8rem', color: '#1628A9', fontWeight: 800, marginBottom: 4 }}>
                        <Plane size={14} /> Vol {course.flightNumber} — <span style={{ color: '#10b981' }}>À l'heure</span>
                     </div>
                   )}
                   {course.luggage && (
                     <div style={{ display: 'flex', items: 'center', gap: 6, fontSize: '0.75rem', color: '#94a3b8' }}>
                        <Luggage size={14} /> {course.luggage}
                     </div>
                   )}
                </div>
              )}
