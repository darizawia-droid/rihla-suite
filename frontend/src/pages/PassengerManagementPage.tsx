        ...
        <div className="col-span-9 space-y-4">
          ...
          {filtered.map(p => (
            <div key={p.id} className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-white/10 p-5 shadow-sm hover:shadow-md transition-all">
              ...
              <div className="grid grid-cols-5 gap-4 mt-4 pt-4 border-t border-slate-100 dark:border-white/5">
                <div>
                  <div className="text-[10px] font-black text-slate-400 uppercase">Profil IA</div>
                  <div className="flex flex-wrap gap-1 mt-1.5">
                    <span className={clsx("px-2 py-0.5 rounded text-[9px] font-bold uppercase", p.id === '1' ? "bg-purple-100 text-purple-700" : "bg-blue-100 text-blue-700")}>
                      {p.id === '1' ? 'Luxury Seeker' : 'Culturalist'}
                    </span>
                    {p.specialRequests && <span className="px-2 py-0.5 bg-amber-100 text-amber-700 rounded text-[9px] font-bold uppercase text-[9px]">High Care</span>}
                  </div>
                </div>
                <div>
                  <div className="text-[10px] font-black text-slate-400 uppercase">Hébergement</div>
                  <div className="text-xs font-bold text-slate-700 dark:text-slate-300 mt-1.5 flex items-center gap-1.5">
                    <Bed size={12} className="text-rihla" /> {p.roomType} {p.roomPartner && <span className="text-[10px] text-slate-400 font-medium">w/ {p.roomPartner}</span>}
                  </div>
                </div>
                <div>
                  <div className="text-[10px] font-black text-slate-400 uppercase">Alimentaire</div>
                  <div className="text-xs font-bold text-slate-700 dark:text-slate-300 mt-1.5 flex items-center gap-1.5">
                    <Utensils size={12} className={p.dietaryRestrictions ? "text-amber-500" : "text-emerald-500"} /> {p.dietaryRestrictions || 'Standard'}
                  </div>
                </div>
                <div>
                  <div className="text-[10px] font-black text-slate-400 uppercase">Santé Passeport</div>
                  <div className="mt-1.5">
                     {(() => {
                        ...
                        return (
                          <div className={clsx("flex items-center gap-1.5 text-[10px] font-bold", diff < 6 ? "text-red-500" : "text-emerald-500")}>
                            <div className={clsx("w-2 h-2 rounded-full", diff < 6 ? "bg-red-500 animate-pulse" : "bg-emerald-500")} />
                            {diff < 6 ? 'Critique' : 'Valide'} ({Math.round(diff)}m)
                          </div>
                        )
                     })()}
                  </div>
                ...
                <div className="flex items-center justify-end gap-2">
                   <button className="p-2.5 bg-slate-50 dark:bg-white/5 rounded-xl text-slate-400 hover:text-rihla hover:bg-rihla/5 transition-all" title="Envoyer WhatsApp de bienvenue">
                      <Phone size={14} />
                   </button>
                   <button className="p-2.5 bg-slate-50 dark:bg-white/5 rounded-xl text-slate-400 hover:text-rihla hover:bg-rihla/5 transition-all" title="Envoyer Rappel Passeport">
                      <Mail size={14} />
                   </button>
                </div>
              </div>
