...
import {
  ...
  ShieldCheck, AlertCircle, Loader2, X, Plus
} from 'lucide-react';
...
function AddAgencyModal({ open, onClose, onSaved }: {
  open: boolean;
  onClose: () => void;
  onSaved: () => void;
}) {
  const [form, setForm] = useState<AddAgencyForm>({
    name: '', email: '', phone: '', country: '', contact_name: ''
  });
  ...
  const mutation = useMutation({
    mutationFn: () => partnersApi.create({
      ...
      currency: 'MAD',
      is_active: true,
      // Store extra fields in a generic way
      code: form.name.substring(0, 8).toUpperCase().replace(/\s+/g, '-') + '-' + Date.now().toString().slice(-4),
    } as any),
    onSuccess: () => { onSaved(); onClose(); setForm({ name: '', email: '', phone: '', country: '', contact_name: '' }); },
  });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm animate-fade-in">
      <div className="bg-white dark:bg-slate-900 rounded-[24px] shadow-2xl w-full max-w-md mx-4 overflow-hidden">
        ...
        <div className="px-6 py-5 border-b border-slate-100 dark:border-white/5 flex items-center justify-between">
          <div>
            <h2 className="font-bold text-lg text-slate-800 dark:text-cream">Ajouter une Agence</h2>
            <p className="text-[12px] text-slate-400 mt-0.5">Nouveau partenaire B2B dans votre réseau</p>
          </div>
        ...
        <div className="p-6 space-y-4">
          <div>
            <label className="block text-[11px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Nom de l'agence *</label>
            <input
              className="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-[13px] text-slate-800 dark:text-cream focus:ring-2 focus:ring-rihla focus:border-rihla transition-all"
              placeholder="Luxe Voyages International"
              value={form.name}
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-[11px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Pays</label>
              <input
                className="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-[13px] text-slate-800 dark:text-cream focus:ring-2 focus:ring-rihla focus:border-rihla transition-all"
                placeholder="France"
                value={form.country}
          ...
          <div>
            <label className="block text-[11px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Téléphone</label>
            <input
              className="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-[13px] text-slate-800 dark:text-cream focus:ring-2 focus:ring-rihla focus:border-rihla transition-all"
          ...
          {mutation.isError && (
            <div className="flex items-center gap-2 p-3 bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/20 rounded-xl text-red-600 dark:text-red-400 text-[12px] font-bold">
              <AlertCircle size={14} /> Erreur lors de la création. Vérifiez les champs.
            </div>
          )}
        </div>
...
export function CrmPage() {
  ...
  const { data: apiPartners, isLoading } = useQuery({
    queryKey: ['partners', 'sub_agent'],
    queryFn: () => partnersApi.list({ type: 'sub_agent' }).then(res => res.data),
  });

  const agencies = useMemo(() => {
    if (!apiPartners || apiPartners.length === 0) return MOCK_AGENCIES;

    const mapped: Agency[] = apiPartners.map(p => ({
      id: p.id,
      name: p.name,
      country: (p.address as any)?.country || 'N/A',
      tier: (p as any).tier || 'Standard',
      contact: (p as any).contact_name || 'Contact Principal',
      email: p.email || 'N/A',
      stats: {
        totalProjects: 0,
        wonProjects: 0,
  ...
  return (
    <div className="min-h-full bg-slate-50 dark:bg-slate-950 transition-colors pb-12">
      <AddAgencyModal
        ...
        onSaved={() => queryClient.invalidateQueries({ queryKey: ['partners', 'sub_agent'] })}
      />
      ...
      <div className="p-8 max-w-[1600px] mx-auto space-y-6">
        ...
        <div className="grid grid-cols-4 gap-4">
          <StatCard 
            ...
            value={agencies.filter(a => a.isReal).length || agencies.length} 
            icon={Building2} 
            variant="dark" 
            sub={agencies.filter(a => a.isReal).length > 0 ? "Données réelles" : "Réseau B2B (DÉMO)"}
          />
          <StatCard 
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-white/5">
                  {isLoading ? (
                    <tr>
                      <td colSpan={5} className="py-20 text-center">
                        <div className="flex flex-col items-center gap-3">
                          ...
                          <p className="text-slate-400 font-bold uppercase tracking-widest text-[10px]">Chargement des partenaires...</p>
                        </div>
                      </td>
                    </tr>
                    ...
                    <tr 
                      ...
                      className={`hover:bg-slate-50 dark:hover:bg-white/5 cursor-pointer transition-colors ${selectedAgencyId === agency.id ? 'bg-rihla/5 dark:bg-rihla/10' : ''}`}
                    >
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <div className={`w-10 h-10 rounded-[10px] flex items-center justify-center font-bold shadow-sm ${agency.isReal ? 'bg-emerald-500/10 text-emerald-600 border border-emerald-500/20' : 'bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-cream'}`}>
                            {agency.name.substring(0, 2).toUpperCase()}
                          </div>
                          <div>
                            <div className="flex items-center gap-2">
                              <p className="font-bold text-slate-800 dark:text-cream">{agency.name}</p>
                              {agency.isReal && <span className="px-1.5 py-0.5 bg-emerald-500 text-white text-[8px] font-black uppercase rounded-md">Live</span>}
                            </div>
            ...
            {selectedAgency ? (
                <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[24px] shadow-sm p-6 sticky top-6 animate-fade-in">
                  <div className="flex items-start justify-between mb-6">
                    <div>
                      <TierBadge tier={selectedAgency.tier} className="mb-3" />
                      <h2 className="font-bold text-xl text-slate-800 dark:text-cream leading-tight">{selectedAgency.name}</h2>
                      <p className="text-[12px] text-slate-400 mt-1">{selectedAgency.id} • {selectedAgency.country}</p>
                    </div>
                    {selectedAgency.isReal && (
                      <span className="px-2 py-1 bg-emerald-500 text-white text-[8px] font-black uppercase rounded-md shrink-0">Live</span>
                    )}
                  </div>

                  {/* Performance mini-stats */}
                  <div className="grid grid-cols-2 gap-3 mb-6">
                    <div className="bg-slate-50 dark:bg-slate-950 rounded-xl p-3 border border-slate-100 dark:border-white/5 text-center">
                      <p className="text-[20px] font-black text-slate-800 dark:text-cream">{selectedAgency.stats.totalProjects}</p>
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Projets</p>
                    </div>
                    <div className="bg-slate-50 dark:bg-slate-950 rounded-xl p-3 border border-slate-100 dark:border-white/5 text-center">
                      <p className={`text-[20px] font-black ${selectedAgency.stats.conversionRate > 70 ? 'text-emerald-500' : selectedAgency.stats.conversionRate > 40 ? 'text-amber-500' : 'text-red-500'}`}>
                        {selectedAgency.stats.conversionRate}%
                      </p>
                  ...
                  <div className="bg-slate-50 dark:bg-slate-950 rounded-xl p-4 mb-6 border border-slate-100 dark:border-white/5 space-y-3">
                    ...
                    <div className="flex items-center gap-3 text-[13px] text-slate-800 dark:text-cream">
                      <div className="w-8 h-8 rounded-full bg-rihla/10 text-rihla flex items-center justify-center font-bold">
                        {selectedAgency.contact.charAt(0)}
                      </div>
                      <div className="flex-1">
                        <p className="font-bold">{selectedAgency.contact}</p>
                        <p className="text-[11px] text-slate-400">{selectedAgency.email}</p>
                      </div>
                      <div className="flex gap-1">
                        <a href={`mailto:${selectedAgency.email}`} className="p-1.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-md hover:text-rihla transition-colors"><Mail size={14} /></a>
                    ...
                    <p className="text-[11px] text-slate-400 pl-11">Dernière activité : {selectedAgency.lastActivity}</p>
                  </div>

                  {/* Preferences / Tags */}
                  {selectedAgency.preferences.length > 0 && (
                    <div className="mb-6">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {selectedAgency.preferences.map((pref, idx) => (
