...
export function DataHubPage() {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      {stats && (
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-3">
          ...
          {Object.entries(stats.by_module).map(([m, c]) => {
            ...
            return (
              <div key={m} className="rounded-lg border border-slate-200 dark:border-slate-700 bg-white p-3">
      ...
      {/* Search box */}
      <form onSubmit={onSubmit} className="bg-white rounded-xl border border-slate-200 dark:border-slate-700 p-4 flex gap-3 items-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {sugg.map((s, i) => (
              <button key={i}
                      ...
                      className="text-left rounded-lg border border-slate-200 dark:border-slate-700 hover:border-violet-300 hover:bg-violet-50 p-3 transition-colors">
        ...
        <div className="grid lg:grid-cols-[240px_1fr] gap-6">
              ...
              <ul className="space-y-1">
                <li>
                  <button onClick={() => applyModuleFilter(null)}
                          className={`w-full flex items-center justify-between px-2 py-1.5 rounded text-sm ${moduleFilter === null ? 'bg-violet-100 text-violet-700' : 'hover:bg-slate-50 dark:bg-slate-950 dark:bg-slate-950'}`}>
                  ...
                  return (
                    <li key={mod}>
                      <button onClick={() => applyModuleFilter(mod)}
                              className={`w-full flex items-center justify-between px-2 py-1.5 rounded text-sm ${moduleFilter === mod ? 'bg-violet-100 text-violet-700' : 'hover:bg-slate-50 dark:bg-slate-950 dark:bg-slate-950'}`}>
          ...
          <div className="space-y-3">
            <div className="text-sm text-slate-500">
              <strong className="text-slate-900 dark:text-white dark:text-white">{results.total}</strong> résultat(s) pour <em>"{results.query}"</em>
              {moduleFilter && <span> · filtré par <strong>{MODULE_META[moduleFilter]?.label ?? moduleFilter}</strong></span>}
            </div>
            {filteredHits.length === 0 && (
              <div className="bg-white rounded-xl border border-slate-200 dark:border-slate-700 p-8 text-center text-slate-500">
            ...
            {filteredHits.map(h => {
              ...
              return (
                <Link key={h.id} to={href}
                      className="block bg-white rounded-xl border border-slate-200 dark:border-slate-700 hover:border-violet-300 hover:shadow-sm transition-all p-4">
                  <div className="flex items-start gap-3">
                    <div className={`w-9 h-9 rounded-lg ${meta?.color ?? 'bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 dark:text-slate-300'} flex items-center justify-center shrink-0`}>
                      <Icon className="w-4 h-4" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        <span className={`text-[10px] uppercase tracking-wider font-semibold px-1.5 py-0.5 rounded ${meta?.color ?? 'bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 dark:text-slate-300'}`}>
                      ...
                      <div className="mt-2 flex flex-wrap gap-3 text-xs text-slate-500">
                        ...
                        {h.amount != null && <span className="font-semibold text-slate-700 dark:text-slate-300 dark:text-slate-300">{fmtMoney(h.amount, h.currency)}</span>}
                        {h.status        && <span className="px-1.5 py-0.5 bg-slate-100 dark:bg-slate-800 rounded">{h.status}</span>}
...
function StatBox({ label, value, accent }: { label: string; value: string; accent?: boolean }) {
  return (
    <div className={`rounded-lg p-3 ${accent ? 'border-2 border-violet-300 bg-violet-50' : 'border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900'}`}>
