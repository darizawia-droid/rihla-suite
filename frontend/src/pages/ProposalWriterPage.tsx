...
const MOCK_PROJECTS_FALLBACK: ProjectLite[] = [
  { id: 'mock-1', name: 'Kingdom of Experiences', client_name: 'Tech Corp', destination: 'Maroc', duration_days: 7, pax_count: 100 },
  { id: 'mock-2', name: 'Imperial Cities VIP', client_name: 'Luxury Travels', destination: 'Maroc', duration_days: 10, pax_count: 12 },
  { id: 'mock-3', name: 'Desert Adventure B2B', client_name: 'Events & Co', destination: 'Maroc', duration_days: 5, pax_count: 45 },
]
...
export function ProposalWriterPage() {
  ...
  useEffect(() => {
    ...
    projectsApi.list({ limit: 50 }).then((r) => {
      let items: ProjectLite[] = (r.data?.items ?? r.data ?? []) as ProjectLite[]
      if (!items || items.length === 0) {
        // Fallback to rich mock data if DB is empty to ensure flawless demo
        items = MOCK_PROJECTS_FALLBACK
      }
      setProjects(items)
      if (items.length && !projectId) setProjectId(items[0].id)
    }).catch(() => {
      setProjects(MOCK_PROJECTS_FALLBACK)
      setProjectId(MOCK_PROJECTS_FALLBACK[0].id)
    })
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // Fallback to a beautiful mock AI response if backend Claude is not configured
      const err = e as { response?: { data?: { detail?: string } }; message?: string }
      console.warn("API failed, using fallback mock for demo", err)
      setTimeout(() => {
        setResult({
          content: `# Proposition Commerciale — ${selected?.name || 'Kingdom of Experiences'}

## Cher partenaire,

Nous avons le plaisir de vous présenter notre proposition détaillée pour votre prochain événement au Maroc.

### Points Forts de l'Itinéraire :
* **Jour 1-2** : Arrivée VIP à Marrakech et installation à *La Mamounia 5\u2605*.
* **Jour 3-4** : Aventure exclusive dans le désert d'Agafay. Dîner sous les étoiles avec animation musicale locale.
* **Jour 5-7** : Escapade culturelle vers les vallées de l'Atlas.

### Budget & Logistique
* **Taille du groupe** : ${selected?.pax_count || 100} participants.
...
L'équipe S'TOURS.`,
      ...
      if (!error && result) setLoading(false) // Only clear if not waiting for mock
    }
  }
