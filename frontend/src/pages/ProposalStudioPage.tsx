...
const TEMPLATES = [
  ...
  { id: 'business', name: "S'TOURS Elite",  color: '#1628A9', font: 'font-sans',  img: 'https://images.unsplash.com/photo-1553508913-264739567433?auto=format&fit=crop&q=80&w=400', accent: 'bg-[#1628A9]' },
]
...
export function ProposalStudioPage() {
  ...
  // ── Real PPTX export via backend ─────────────────────────────────
  const handleGenerate = async () => {
    if (exportFormat === 'pdf') {
      // PDF fallback — browser print
      setIsGenerating(true)
      setTimeout(() => { setIsGenerating(false); window.print() }, 800)
      return
    }
    ...
    try {
      ...
      const payload = {
        ...
        single_supplement: sim.singleSupplement,
      }
      ...
      a.href     = url
      a.download = `STOURS_Proposal_Magical_Morocco.pptx`
      ...
      setExportError(err.message || 'Génération échouée — vérifiez que Node.js est installé sur le serveur.')
    } finally {
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="bg-white border-b border-slate-200 px-8 py-4 flex justify-between items-center sticky top-0 z-50 shadow-sm">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex bg-slate-100 p-1 rounded-xl border border-slate-200">
            <button
              ...
              className={clsx(
                ...
                exportFormat === 'pptx' ? 'bg-white text-rihla shadow-sm' : 'text-slate-400 hover:text-slate-600',
              )}
            >
              PPTX Presentation
            </button>
            <button
              ...
              className={clsx(
                'px-4 py-2 text-[10px] font-black uppercase tracking-widest rounded-lg transition-all',
                exportFormat === 'pdf' ? 'bg-white text-rihla shadow-sm' : 'text-slate-400 hover:text-slate-600',
              )}
            >
              PDF (Print)
            </button>
          ...
          {/* Error badge */}
          {exportError && (
            <div className="flex items-center gap-1.5 px-3 py-1.5 bg-red-50 border border-red-100 rounded-xl text-[10px] font-bold text-red-600 max-w-xs truncate">
          ...
          <button
            ...
            className="flex items-center gap-2 px-6 py-2.5 bg-ink text-cream text-xs font-bold rounded-xl shadow-xl hover:-translate-y-0.5 transition-all disabled:opacity-50"
          >
            {isGenerating
              ? <><Loader2 size={14} className="animate-spin" /> Génération…</>
              : <><Download size={14} /> Télécharger {exportFormat.toUpperCase()}</>
            }
          </button>
          <div className="h-8 w-px bg-slate-200 mx-2" />
          <button className="flex items-center gap-2 px-4 py-2.5 bg-rihla text-white text-xs font-bold rounded-xl shadow-lg hover:shadow-rihla/30 transition-all">
      ...
      <div className="max-w-[1400px] mx-auto px-8 py-8 grid grid-cols-12 gap-8">
        ...
        <div className="col-span-3 space-y-6">
          ...
          {/* AI Assistant */}
          <div className="bg-rihla/5 border border-rihla/10 rounded-2xl p-5">
             <div className="flex items-center gap-2 mb-3">
                <Sparkles size={16} className="text-rihla" />
                <span className="text-[11px] font-black uppercase text-rihla">IA Design Assistant</span>
             </div>
             <p className="text-[11px] text-slate-600 italic leading-relaxed">
               "Les images du Jour 4 sont trop denses. Suggérer un layout 'Hero' pour aérer le document ?"
             </p>
             <button className="mt-3 w-full py-2 bg-rihla text-white text-[10px] font-black uppercase rounded-lg">Optimiser le flux</button>
          </div>
