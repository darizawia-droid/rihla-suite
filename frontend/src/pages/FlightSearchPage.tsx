...
const RETURN_FLIGHTS: FlightResult[] = [
...
export function FlightSearchPage() {
  ...
  const sortedFlights = [...MOCK_FLIGHTS]
  ...
  const selectedOut = MOCK_FLIGHTS.find(f => f.id === selectedOutbound)
  const selectedRet = RETURN_FLIGHTS.find(f => f.id === selectedReturn)
        ...
        <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-white/10 p-6 shadow-sm">
          <div className="grid grid-cols-6 gap-4">
            ...
            <div className="flex items-end">
              <button className="w-full px-4 py-2.5 bg-rihla text-white rounded-xl text-sm font-black uppercase shadow-lg shadow-rihla/20 hover:bg-rihla/90 transition-all flex items-center justify-center gap-2">
