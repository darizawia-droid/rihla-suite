...
export function AllotmentManagerPage() {
  ...
  const confirmAllotment = (id: string) => {
    setAllotments(prev => prev.map(a =>
      a.id === id ? { ...a, roomsConfirmed: a.roomsBlocked, status: 'confirmed' as const } : a
    ))
  }

  const releaseAllotment = (id: string) => {
    setAllotments(prev => prev.map(a =>
      a.id === id ? { ...a, roomsReleased: a.roomsBlocked - a.roomsConfirmed, status: a.roomsConfirmed > 0 ? 'partial' as const : 'released' as const } : a
    ))
  }
