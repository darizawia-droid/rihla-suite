...
<head>
  <meta charset="UTF-8">
  <style>
    .staff-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 40px; margin-top: 20px; }
    .staff-card { background: #f8fafc; padding: 20px; border-radius: 12px; display: flex; align-items: center; gap: 20px; }
    .staff-card img { width: 80px; height: 80px; border-radius: 50%; object-fit: cover; border: 3px solid #C5943A; }
    .staff-info h4 { margin: 0; color: #1B2A4A; font-size: 16px; }
    .staff-info p { margin: 5px 0 0; color: #64748b; font-size: 12px; }
    {{ css }}
  </style>
</head>
<body>
...
<!-- ═══ FIELD STAFF ═══ -->
{% if project.assigned_guide or project.assigned_driver %}
<h2 class="section-title">Votre Équipe Dédiée</h2>
<p>Pour garantir une expérience sans faille, nous avons sélectionné nos meilleurs experts pour vous accompagner durant votre séjour.</p>

<div class="staff-grid">
    {% if project.assigned_guide %}
    <div class="staff-card">
        <img src="{{ project.assigned_guide.image_url }}" alt="Guide">
        <div class="staff-info">
            <h4>{{ project.assigned_guide.name }}</h4>
            <p>Votre Guide Expert</p>
            <p style="font-size: 10px;">{{ project.assigned_guide.email }}</p>
        </div>
    </div>
    {% endif %}

    {% if project.assigned_driver %}
    <div class="staff-card">
        <img src="{{ project.assigned_driver.image_url }}" alt="Chauffeur">
        <div class="staff-info">
            <h4>{{ project.assigned_driver.name }}</h4>
            <p>Votre Chauffeur Professionnel</p>
            <p style="font-size: 10px;">{{ project.assigned_driver.email }}</p>
        </div>
    </div>
    {% endif %}
</div>
