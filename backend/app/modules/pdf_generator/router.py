...
def _load_project_data(db: Session, project_id: str) -> dict:
    ...
    project = db.execute(
        ...
        .options(
            selectinload(Project.assigned_guide),
            selectinload(Project.assigned_driver)
        )
    ).scalars().first()
    if not project:
    ...
    return {
        "project": {
            ...
            "assigned_guide": {
                "name": project.assigned_guide.full_name,
                "email": project.assigned_guide.email,
                "image_url": getattr(project.assigned_guide, 'image_url', None) or "https://ui-avatars.com/api/?name=" + project.assigned_guide.full_name.replace(" ", "+")
            } if project.assigned_guide else None,
            "assigned_driver": {
                "name": project.assigned_driver.full_name,
                "email": project.assigned_driver.email,
                "image_url": getattr(project.assigned_driver, 'image_url', None) or "https://ui-avatars.com/api/?name=" + project.assigned_driver.full_name.replace(" ", "+")
            } if project.assigned_driver else None,
        },
        "days": days,
        "quotation": quotation,
    }
