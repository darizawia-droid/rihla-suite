...
class Passenger(BaseModel):
    ...
    # Flights
    arrival_flight: Optional[str] = None    # e.g. AT761
    arrival_time: Optional[str] = None      # e.g. 2024-05-20T14:30
...
def export_for_rooming(project_id: str, db: Session = Depends(get_db)):
    ...
    for p in passengers:
        formatted.append({
            ...
            "notes": p.get("notes", ""),
            "flights": {
                "arrival": p.get("arrival_flight"),
                "arrival_time": p.get("arrival_time"),
                "departure": p.get("departure_flight"),
