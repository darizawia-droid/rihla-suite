...
def post_checkin(
    ...
    """Register a real-time check-in from the field."""
    from app.modules.field_ops.models import FieldCheckIn
    user_id = current_user.get("sub") or current_user.get("id")
    ...
    db.add(checkin)
    db.commit()
    return {"id": checkin.id, "status": "checked_in"}


@router.get("/projects/{project_id}/logbook/pdf")
def export_logbook_pdf(
    project_id: str,
    db: Session = Depends(get_db)
):
    """Generate a final digital logbook PDF for a project."""
    from app.modules.projects.models import Project
    from app.modules.field_ops.models import FieldCheckIn, FieldIncident, FieldTask
    ...
    tasks = db.execute(select(FieldTask).where(FieldTask.project_id == project_id)).scalars().all()
    checkins = db.execute(select(FieldCheckIn).where(FieldCheckIn.project_id == project_id)).scalars().all()
    incidents = db.execute(select(FieldIncident).where(FieldIncident.project_id == project_id)).scalars().all()

    # In a real app, use WeasyPrint or ReportLab here.
    # For the demo, we return a mock PDF content.
    content = f"%PDF-1.4\n% RIHLA DIGITAL LOGBOOK - Project: {project.name}\n"
    content += f"Summary: {len(tasks)} tasks, {len(checkins)} check-ins, {len(incidents)} incidents.\n"
    content += "Status: CIRCUIT COMPLETED SUCCESSFULY"
    
    return Response(
        content=content.encode(),
        media_type="application/pdf",
        headers={"Content-Disposition": f"attachment; filename=logbook_{project_id}.pdf"}
    )
