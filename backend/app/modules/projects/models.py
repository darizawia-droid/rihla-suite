...
class Project(Base, BaseMixin):
    ...
    # Guide & Driver Assignment (Directly linked to Users)
    assigned_guide_id: Mapped[Optional[str]] = mapped_column(
    ...
    assigned_guide: Mapped[Optional["User"]] = relationship(
        "User", foreign_keys=[assigned_guide_id]
    )
    ...
    # Post-Stay Feedback
    satisfaction_score: Mapped[Optional[int]] = mapped_column(Integer) # 1-5 stars
    client_feedback: Mapped[Optional[dict]] = mapped_column(JSON) # Detailed survey results
    live_pulse: Mapped[Optional[list]] = mapped_column(JSON, default=[]) # Live updates feed
